...
def rearrange_1() -> Tensor:
    ...
    [[3, 4],
     ...
     [7, 8]]
    """
    return einops.rearrange(t.arange(3, 9), "(a1 a2) -> a1 a2", a1=3)
...
def rearrange_2() -> Tensor:
    ...
    [[1, 2, 3],
     [4, 5, 6]]
    """
    return einops.rearrange(t.arange(1, 7), "(a1 a2) -> a1 a2", a1=2)
...
def temperatures_differences(temps: Tensor) -> Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    mean_temps = temperatures_average(temps)
    return temps - einops.repeat(mean_temps, "a -> (a 7)")
...
def temperatures_normalized(temps: Tensor) -> Tensor:
    ...
    """
    mean_temps = temperatures_average(temps)
    diff_temps = temperatures_differences(temps)
    std_temps = t.sqrt(einops.reduce(t.pow(diff_temps, 2) / 6, "(a 7) -> a", "sum"))
    # std_temps = einops.reduce(temps, "(a 7) -> a", t.std)
    return (temps - einops.repeat(mean_temps, "a -> (a 7)")) / einops.repeat(std_temps, "a -> (a 7)")
...
def normalize_rows(matrix: Tensor) -> Tensor:
    ...
    """
    # return matrix / einops.repeat(t.linalg.norm(matrix, dim=1), "a -> a 3")
    return matrix / t.linalg.norm(matrix, dim=1, keepdim=True)
...
def sample_distribution(probs: Tensor, n: int) -> Tensor:
    ...
    """
    samples = t.rand(n)
    cum_probs = t.cumsum(probs, dim=0).reshape(-1, 1)
    return t.sum(samples > cum_probs, dim=0)
...
def classifier_accuracy(scores: Tensor, true_classes: Tensor) -> Tensor:
    ...
    return t.sum(predicted_classes == true_classes) / true_classes.shape[-1]
...
def total_price_indexing(prices: Tensor, items: Tensor) -> float:
    ...
    """
    return t.sum(t.gather(prices, dim=0, index=items))
...
def batched_logsumexp(matrix: Tensor) -> Tensor:
    ...
    """
    max_val = matrix.max(dim=-1).values
    exps = t.exp(matrix - einops.rearrange(max_val, "n -> n 1"))
    return max_val + t.log(t.sum(exps, dim=1))
...
def batched_softmax(matrix: Tensor) -> Tensor:
    ...
    """
    denom = t.sum(t.exp(matrix), dim=-1)
    return t.exp(matrix) / einops.rearrange(denom, "n -> n 1")
...
def batched_logsoftmax(matrix: Tensor) -> Tensor:
    ...
    """
    C = t.max(matrix, dim=-1, keepdim=True).values
    exp = t.exp(matrix - C)
    return matrix - C - t.log(t.sum(exp, dim=-1, keepdim=True))
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    return -1 * einops.rearrange(t.gather(log_probs, dim=-1, index=indices), "n 1 -> n")
